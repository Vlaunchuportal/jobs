...
if(isset($_GET['email']) && isset($_GET['reset']))
{
    ...
    $date=date("y-m-d");
    $sql = "SELECT * FROM candidate WHERE cemail = '$_GET[email]' AND resetToken = '$_GET[reset]' AND resetExpire = '$date' ";
    $result = $conn->query($sql);

    $sql = "SELECT * FROM recruiter WHERE remail = '$_GET[email]' AND resetToken = '$_GET[reset]' AND resetExpire = '$date' ";
    ...
    if($result->num_rows == 1)
    {
        if($result->num_rows == 1) 
        {
            if(isset($_POST['send']))
            {
                ...
                $sql = "UPDATE candidate SET cpassword = '{$_POST['pass']}' WHERE cemail = '$_GET[email]'";
                $result = $conn->query($sql);
                echo "<script>
    ...
    else if($resultr->num_rows == 1)
    {
        if($resultr->num_rows == 1) 
        {
            if(isset($_POST['send']))
            {
                ...
                $sql = "UPDATE recruiter SET rpassword = '{$_POST['pass']}' WHERE remail = '$_GET[email]'";
                $result = $conn->query($sql);
                echo "<script>
                alert('password changed successfully');
                window.location.href='index.php'; </script>";
        ...
        <div class="col-md-6 contents">
          <div class="row justify-content-center">
            <div class="col-md-8">
              ...
              <p class="mb-4"><strong>Enter new password</strong></p>
            </div>
            <form  method="POST" action="" >
              ...
              <button class="btn text-white btn-block btn-primary" type="submit" name="send" >Change password</button>
