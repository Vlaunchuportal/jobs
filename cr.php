...
<head>
    ...
    <meta name="description" content="Consilt - Business HTML Template" />
    <meta name="keywords" content="accountant, advertising, adviser, business, company" />
...
try {
  ...
  <a href='http://localhost/job-listing-master/verify.php?cemail=$cemail&vcode=$vcode'>Verify</a>";
...
{
    $sql="SELECT * FROM candidate WHERE cemail = '{$_POST['cemail']}'";
    $result = $conn->query($sql);

    $sqlr="SELECT * FROM recruiter WHERE remail = '{$_POST['cemail']}'";
    ...
    else
    {
        ...
        if($_POST['code']!='')
        {
        $sql = "SELECT * FROM candidate where referralcode = '$cn'";
        ...
        $result = $conn->query($sql);
            ...
            if($result->num_rows == 1) 
            {
                ...
                $sql = "UPDATE candidate SET wallet = '$wallet' WHERE referralcode = '$cn' ";
                $result = $conn->query($sql);
        ...
        $sql = "INSERT INTO candidate(cname,cemail,cpassword,cnum, usertype, verificationCode, isVerified,news, referralcode) VALUES('$cname','$cemail','$cpassword','$cnum', 'candidate', '$vcode', '0', '$ss', '$referralcode')";
        ...
        <div class="col-md-6 contents">
          <div class="row justify-content-center">
            <div class="col-md-8">
              ...
              <p class="mb-4">Candidate account</p>
            </div>
            ...
            if(!isset($_SESSION['is_login']))
            {
              ...
              {   
                $sql = "SELECT * FROM candidate where cemail = '{$_SESSION['user_email_address']}' ";
                $result = $conn->query($sql);
                $row = $result->num_rows;

                $sqlre = "SELECT * FROM recruiter where remail = '{$_SESSION['user_email_address']}' ";
                $resultre = $conn->query($sqlre);

                if($resultre->num_rows == 1) 
                {
                  echo "<script>
                  alert('Already registered email in recruiter account');
                ...
                if($row === 1 ) 
                {
                  ...
                  window.location.href='candidate/Candidate.php'; </script>";
                }
                
                else
                {

                  $sql= "INSERT INTO candidate (cname, cemail,cimage, usertype) VALUES ('$sname', '$semail', '$simage', 'candidate')";
