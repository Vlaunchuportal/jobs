					...
					<div class="row justify-content-center d-flex">
						<div class="col-lg-8 post-list">
								...
								<div class="details">
									<div class="title d-flex flex-row justify-content-between">
										...
										<div class="titles">
											...
											<h6>Reported Candidate id: <?php echo $row['canid']; ?></h6>					
										</div>
										<!-- <ul class="btns">
									...
									<p class="address">Reported by Recruiter id: <?php echo $row['recid']; ?> </p>
                                    ...
                                    <ul class="btns">
                                        ...
                                        <form action="" method="POST" class="d-inline">
										<input type="hidden" name="cid" value="<?php echo $row['canid']; ?>">
										<button type="submit" class="btn job-view bt btn-danger" name="cdelete" value="cdelete">Delete candidate account</button>
										</form>

										<form action="" method="POST" class="d-inline">
										<input type="hidden" name="viewid" value="<?php echo $row['reportingid']; ?>">
										<button type="submit" class="btn job-view b btn-secondary" name="view" value="view">Suspend candidate</button>
										</form>
										</ul>
								</div>
                            ...
                            if(isset($_REQUEST['delete'])) {
                                $sql = "DELETE FROM canreporting where reportingid = {$_REQUEST['id']}";
                            ...
                            if(isset($_REQUEST['cdelete'])) {
                                $sql = "DELETE FROM candidate where cid = {$_REQUEST['cid']}";
                                if($conn->query($sql) == TRUE){
                                    echo '<meta http-equiv="refresh" content="0;URL=?deleted" />';
                                } else {
                                    echo "unable to delete data";
